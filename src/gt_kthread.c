...
/// uthread shared info
//uthread_shared_info_t *uthread_shared_info[NUM_THREADS];
...
//static inline void uthread_info_init();
...
#define k_ctx ((kthread_context_t *)arg)
	...
	kthread_init(k_ctx);
#if 0
	printf("\nAfter kthread_init Thread (tid : %u, pid : %u,  cpu : %d, cpu-apic-id %d) ready to run !!\n\n", 
...
{
//	printf("Inside ksched_find_target\n");
...
static void gtthread_app_start(void *arg)
{
//	printf("Inside gtthreaD_app_start\n");
	...
	// TODO:load-balancing. check other queues if they have more than one thread, then run them. 
	kthread_exit();
...
{
//	printf("Inside gtthread_app_init\n");
...
//	printf("Before ksched INFO INIT\n");
	...
	// Initialise uthread_shared_info
	//uthread_shared_info_init(&uthread_shared_info);
...
{
//	printf("Inside gtthread_app_exit\n");
	...
	// TODO: load balancing. There are two places where load balancing can be done. Find the most appropriate one.
	// This one is only for load-balancing by master thread. Other one is for load-balancing by other threads. So, we should do it at both places.
	kthread_block_signal(SIGVTALRM); // Why do we block signals here?
//	kthread_block_signal(SIGUSR1);
...
/*void print_statistics() 
{
	...
	{	
		printf("kthread id: %d, group_id: %d, elapsed_time: %d\n",uthread_shared_info[i]->thread_id,
			uthread_shared_info[i]->group_id, uthread_shared_info[i]->elapsed_time);
	}	
	return;
}*/
